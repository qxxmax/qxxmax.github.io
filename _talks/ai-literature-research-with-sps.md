---
title: "How to Play with a Child Holding Mythic Weapons: AI Literature Research"
title_zh: "如何和拥有神话武器的小孩一起玩玩具：用 AI 做文献调研"
date: 2026-07-15
event: "Helmholtz Institute Mainz (HIM)"
pdf: /assets/pdf/talks/ai-literature-research-sps-moxian-qian-en.pdf
external_pdf: /assets/pdf/talks/ai-literature-research-sps-moxian-qian-en.pdf
description: <span class="en">An auditable SPS case study covering target lock, multi-round search, paper reading, Dijkstra ranking, citation lineage, and claim boundaries. <a href="/assets/pdf/talks/ai-literature-research-sps-moxian-qian-zh.pdf" target="_blank" rel="noopener">Chinese slides</a>.</span><span class="zh">以 SPS 为案例，展示对象确认、多轮搜索、逐篇阅读、Dijkstra 排序、引用谱系与结论边界。<a href="/assets/pdf/talks/ai-literature-research-sps-moxian-qian-zh.pdf" target="_blank" rel="noopener">中文 slides</a>。</span>
---

<div class="en" markdown="1">

The deck shows the actual Excel source table, paper-reading records, checked
citation lineage, and claim/evidence boundary produced by the workflow.

[English slides](/assets/pdf/talks/ai-literature-research-sps-moxian-qian-en.pdf) ·
[Chinese slides](/assets/pdf/talks/ai-literature-research-sps-moxian-qian-zh.pdf)

</div>

<div class="zh" markdown="1">

这套 slides 直接展示流程生成的 Excel 来源表、逐篇阅读记录、核查后的引用谱系，以及
claim/evidence 边界。

[英文 slides](/assets/pdf/talks/ai-literature-research-sps-moxian-qian-en.pdf) ·
[中文 slides](/assets/pdf/talks/ai-literature-research-sps-moxian-qian-zh.pdf)

</div>
